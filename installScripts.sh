...
# figma-* 는 Figma 플러그인 프로젝트(manifest.json + 캔버스 안에서 도는 code.js) —
# node CLI 가 아니므로 ~/bin wrapper 대상이 아님. webExporter/Markdown2ID 와 동급.
SKIP_DIRS=("webExporter" "Markdown2ID" "memory" "backup" ".git" ".claude" "PDF to JPEG.app" "PDF to JPEG.workflow" "tools" "cli" "figma-brand-guide" "figma-pdf-grid")
...
install_scripts() {
    ...
    # 미래에 도구를 tools/ 에서 삭제해도 ~/bin 에 stale entry 안 남게.
    local cleaned=0
    for entry in "$dst_dir"/*; do
        [ -e "$entry" ] || [ -L "$entry" ] || continue
        # 케이스 1: dead symlink (target 파일 없음)
        if [ -L "$entry" ] && [ ! -e "$entry" ]; then
            rm -f "$entry"
            echo "  [clean]  $(basename "$entry") — dead symlink"
            ((cleaned++))
            continue
        fi
        # 케이스 2: 우리가 생성한 wrapper 인데 Source 파일이 사라진 경우
        if [ -f "$entry" ] && [ ! -L "$entry" ]; then
            if grep -q '^# Auto-generated by installScripts.sh' "$entry" 2>/dev/null; then
                local src
                src="$(sed -n 's/^# Source: //p' "$entry" | head -1)"
                if [ -n "$src" ] && [ ! -f "$src" ]; then
                    rm -f "$entry"
                    echo "  [clean]  $(basename "$entry") — source 없음 ($src)"
                    ((cleaned++))
                fi
            fi
        fi
    ...
    echo "  완료: wrapper ${count}개, cleanup ${cleaned}개"
}
