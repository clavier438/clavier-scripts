 ...
 *   FRAMER_WORKER      어느 워커의 Doppler config 사용 (sisoso/mukayu, 기본 sisoso)
 *                      - sisoso → Doppler[prd]
 *                      - mukayu → Doppler[prd_mukayu]
...
const DEFAULT_BASE = process.env.AIRTABLE_BASE_ID ?? "appIrjwfcGeVMI9xb"   // dev fallback (Doppler 미주입 시)

// 워커 선택 — FRAMER_WORKER env 또는 디폴트 sisoso. Doppler config 자동 라우팅 (workerEnvMap.mjs SSOT).
const FRAMER_WORKER = process.env.FRAMER_WORKER ?? "sisoso"
const WORKER_ENV = getWorkerEnv(FRAMER_WORKER) ?? getWorkerEnv("sisoso")
const DOPPLER_CONFIG = WORKER_ENV.dopplerConfig
...
function dop(args, opts = {}) {
    ...
    // FRAMER_WORKER env 가 결정한 dopplerConfig 사용 (sisoso=prd, mukayu=prd_mukayu)
    return execSync(
        `doppler run --project clavier --config ${DOPPLER_CONFIG} -- npx tsx src/platform/node/cli.ts ${args}`,
...
const commands = {
    ...
    server(args) {
        ...
        const subs = {
            start() {
                ...
                const child = spawn("doppler", ["run", "--project", "clavier", "--config", DOPPLER_CONFIG, "--",
    ...
    deploy(args) {
        // env 인자가 워커 이름 — workerEnvMap 의 dopplerConfig 사용 (sisoso=prd, mukayu=prd_mukayu)
        const env = args[0] ?? "sisoso"
        const wenv = getWorkerEnv(env)
        if (!wenv) {
            console.log(red(`✗ 알 수 없는 워커: ${env}`))
            console.log(dim(`  사용 가능: sisoso, mukayu (workerEnvMap.mjs)`))
            process.exit(2)
        }
        execSync(`doppler run --project clavier --config ${wenv.dopplerConfig} -- npm run deploy:${env}`,
