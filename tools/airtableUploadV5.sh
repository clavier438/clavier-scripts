#!/bin/bash
# schema.json + CSV 폴더 → Airtable 새 베이스 생성 (v5: FML/LKP/RLP 지원)
#
# 사용법:
#   airtableUploadV5 [folder] [--base NAME] [--workspace WS_ID]
#
# 예시:
#   airtableUploadV5 ~/Downloads/sisoso_1.0.1
#   airtableUploadV5 ~/Downloads/sisoso_1.0.1 --base "새프로젝트"
...
echo "▸ base: $BASE  version: $VER  [v5 — FML/LKP/RLP 지원]"
echo "▸ 폴더: $FOLDER"
[[ -n "$*" ]] && echo "▸ 옵션: $*"
echo ""

cd "$FOLDER"
python3 "$SELF_DIR/airtableGenericV5.py" "$@"
