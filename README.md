...
> **포맷 후 복구 한 줄 요약** (2026-05-02 갱신)
> ```bash
> bash ~/Library/Mobile\ Documents/com\~apple\~CloudDocs/0/scripts/setup.sh
> doppler login                                                # 시크릿 복구 (브라우저 인증)
> workerCtl sisoso                                             # framer-sync 워커 호출 (push)
> ```
> setup.sh 가 node·jq·doppler·workerCtl 까지 자동.

**framer-sync 일상 운영 = `workerCtl sisoso → 2 (push)`** (2026-05-02~ 미니멀):
- 워커 1개 (framer-sync-sisoso) — Cloudflare Worker 단일.
- time-budget streaming — 한 번 트리거 후 함수가 끝까지 책임. records 변동성 자동 적응.
- workerCtl 이 capabilities.streamingStep 보고 done=true 까지 자동 step loop.
- cron 매분 백업 — Mac 닫혀도 끝까지.
- 시스템 도면: `clavier-hq/MAP.md`. 구조 한눈에.
...
**반복 실수 차단 hook (2026-05-28~)** — **reference-class agent hook**:
- `tools/claude-hooks/pre-tool-use.agent-reference-class.md` (verifier prompt)
- 외부 도구 (Framer 등) 코드 작성 시도 시 별도 Claude 인스턴스가 트랜스크립트 검사 → reference 부재면 차단
- clavier-hq DECISIONS 2026-05-28 ADR 참조

**환경 모델 (2026-05-03~ + 콜로니 self-install 2026-05-24~)** — **environment-peer**:
...
- Layer 1 도구 5개 (workerCtl / closer-runner / doc-coverage / doppler-sync-wrangler / ociIn) 가 **sibling-first** 자동 탐색 — env > sibling > iCloud Mac fallback. 어느 환경에서 호출해도 zero-config.
- **Mac peer 표준 콜로니 위치 = `~/dev/clavier/`** (4 repo sibling). 호스트 어댑터 install = `bash clavier-hq/bootstrap.sh ensure` (멱등). `tools/claude-hooks/<event>.sh` 폴더가 settings.json hook 등록 *정의* (SvelteKit 정신). `hooks/post-merge` 가 `git pull` 직후 ensure 자동.
- 자세한 결정: `clavier-hq/DECISIONS.md` 2026-05-03 / 2026-05-24 ADR.
...
**framer-sync / control-tower 구조 점검 (2026-04-30)**: D1 SSOT 마이그레이션 직후 SOLID 감사 완료. 주요 발견: `syncCollectionNative ↔ syncCollectionFromD1` 중복(높음), KV 사망 파라미터·모놀리식 라우터(중간). 시정 4개 CLOSER_QUEUE.md 등록. 세부 내용: clavier-hq/DECISIONS.md 2026-04-30.
