...
스크립트는 멱등 — 재실행 안전. 부트 후 `~/clavier-scripts`, `~/clavier-hq`, `~/platform-workers` 가 형제로 배치되어 Layer 1 도구(workerCtl, doc-coverage, doppler-sync-wrangler, closer-runner) 가 zero-config 작동. 자세한 단계는 `bootstrap-agent.sh` 헤더 참조.
