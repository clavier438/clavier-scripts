...
#   - 어떤 repo 도 자동 clone 하지 않음 (에이전트가 작업 시점에 결정)
#
# 사용법 (OCI 서버 SSH 세션에서):
...
echo "    2. 작업할 repo clone (예시):"
echo "       git clone https://github.com/clavier0/clavier-hq      ~/clavier-hq"
echo "       git clone https://github.com/clavier0/platform-workers ~/platform-workers"
echo "    3. 에이전트 실행:"
echo "       cd ~/<repo> && doppler run -- claude"
echo ""
echo "  모바일에서 단발 작업 트리거 (Mac 의 oci-connect 같은 방식):"
