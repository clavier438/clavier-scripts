#!/usr/bin/env python3
"""
syncObsidian.py — Obsidian vault → Google Drive API 직접 싱크
트리거: LaunchAgent(com.clavier.watcherObsidian) 또는 직접 실행
@group watcher
@type launchagent
@label com.clavier.watcherObsidian

직접 실행:
  python3 ~/bin/daemons/syncObsidian
...
# ── 설정 ─────────────────────────────────────────────────────────────────────
VAULT_PATH   = Path.home() / "Library/Mobile Documents/iCloud~md~obsidian/Documents"
GDRIVE_ROOT  = "obsidianSync"
CACHE_FILE   = Path.home() / ".cache/syncObsidian.json"
LOG_FILE     = Path.home() / "Library/Logs/syncObsidian.log"
SECRETS_FILE = Path.home() / ".config/clavier/secrets"

EXCLUDE_NAMES = {".DS_Store", ".trash", ".obsidian", ".sync.pid", ".sync.log"}
EXCLUDE_EXTS  = {".icloud"}   # iCloud 스텁: 다운로드 전 상태
...
def build_drive_snapshot(drive: GDrive, folder_id: str, prefix: str,
                         snapshot: dict):
    """obsidianSync/ 를 재귀 순회해 {rel_path: {drive_id, md5}} 빌드."""
...
# ── 락 파일 (중복 실행 방지) ─────────────────────────────────────────────────
LOCK_FILE = Path("/tmp/syncObsidian.lock")

def acquire_lock() -> bool:
    if LOCK_FILE.exists():
        try:
            pid = int(LOCK_FILE.read_text().strip())
            os.kill(pid, 0)   # 프로세스 살아있는지 확인
            return False       # 이미 실행 중
        except (ProcessLookupError, ValueError, OSError):
            pass              # 죽은 프로세스 → 락 파일 무효
...
# ── 핵심 싱크 로직 ────────────────────────────────────────────────────────────
def sync(drive: GDrive, cache: dict) -> dict:
    log.info("=== syncObsidian 시작 ===")
    ...
    # 폴더 ID 맵: rel_path(빈 문자열=root) → drive folder id
    ...
    # os.walk: 부모 폴더를 자식보다 먼저 처리 (topdown=True)
    for dirpath, dirnames, filenames in os.walk(VAULT_PATH, topdown=True):
        # 제외 폴더 필터 (in-place 수정으로 os.walk 가지치기)
        dirnames[:] = [d for d in dirnames if not should_skip(Path(dirpath) / d)]

        dir_path = Path(dirpath)
        rel_dir = str(dir_path.relative_to(VAULT_PATH))
        if rel_dir == ".":
            rel_dir = ""
        ...
        if rel_dir:
            ...
            if rel_dir not in folder_ids:
                parent_rel = str(dir_path.parent.relative_to(VAULT_PATH))
                if parent_rel == ".":
                    parent_rel = ""
                parent_id = folder_ids.get(parent_rel, root_id)
                fid = drive.get_or_create_folder(dir_path.name, parent_id)
                folder_ids[rel_dir] = fid
                cache[folder_cache_key] = {"drive_id": fid}

        parent_id = folder_ids.get(rel_dir, root_id)
        ...
        for fname in sorted(filenames):
            ...
            try:
                new_id = drive.upload(content, fname, parent_id, existing_id)
