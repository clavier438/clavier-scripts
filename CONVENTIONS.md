...
> 이 섹션이 environment-peer 모델의 "위생" 측면. Layer 1 도구 자체는 **sibling-first** 자동 탐색 (env > sibling) 이라 환경변수 export 없이 어느 peer 에서도 작동 — 공유 헬퍼 `tools/lib/repoPaths.mjs` (.mjs) / `tools/lib/repoPaths.sh` (.sh) + `ARCHITECTURE.md` "이 repo 안 파일의 Layer 분류" 표 참조. 결정 본문: clavier-hq DECISIONS 2026-05-03 (+ iCloud fallback 폐기 2026-05-30).
...
- `clavier-hq/briefings/`: 주간 Architect 브리핑
...
- **Layer 3** — Architect 주간 책무 #4 (마지막 그물)
