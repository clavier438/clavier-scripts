...
> ⚠️ **2026-05-13 STALE 마크** — 본 명세의 "기존 워커엔 없으니 paste" 가정이 baseline 점검 후 사실과 다름. 워커는 이미 `POST /configure` 안에서 webhook 자동 등록 + `POST /webhook` 라우트 ingest + cron 5일 refresh 모두 처리 중. 본 명세는 *paste 가이드* 가 아니라 **향상 가이드** (HMAC 검증 + partial sync 도입) 로 재해석하라. 자세히 = `HANDOFF.md` §"⚠️ 2026-05-13 baseline 점검 후 사실관계 정정".
